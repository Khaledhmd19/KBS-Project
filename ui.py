...
from constants import (
    ...
    LEFT_PANEL_W,
    APP_BG, CARD_BG, PANEL_BG, ACCENT, TEXT_MAIN, TEXT_MUTED,
    FONT_FAMILY,
    FONT_TITLE_SIZE, FONT_SECTION_SIZE, FONT_NORMAL_SIZE, FONT_SMALL_SIZE,
    FONT_WEIGHT_BOLD, FONT_WEIGHT_NORMAL,
    PAD_OUTER, PAD_INNER, PAD_WIDGET,
    APP_SUBTITLE, APP_DESCRIPTION,
    REPORT_TITLE, REPORT_HINT,
    BTN_ANALYZE, BTN_RESET, BTN_COPY,
    TAB_PROBLEM, TAB_DATASET, TAB_FEATURES, TAB_QUALITY, TAB_TRAINING,
)
...
class NeuralNetworkExpertApp(customtkinter.CTk):
    ...
    def __init__(self):
        ...
        self.font_title   = customtkinter.CTkFont(family=FONT_FAMILY, size=FONT_TITLE_SIZE,   weight=FONT_WEIGHT_BOLD)
        self.font_section = customtkinter.CTkFont(family=FONT_FAMILY, size=FONT_SECTION_SIZE, weight=FONT_WEIGHT_BOLD)
        self.font_normal  = customtkinter.CTkFont(family=FONT_FAMILY, size=FONT_NORMAL_SIZE,  weight=FONT_WEIGHT_NORMAL)
        self.font_small   = customtkinter.CTkFont(family=FONT_FAMILY, size=FONT_SMALL_SIZE,   weight=FONT_WEIGHT_NORMAL)
        self.font_bold    = customtkinter.CTkFont(family=FONT_FAMILY, size=FONT_NORMAL_SIZE,  weight=FONT_WEIGHT_BOLD)
    ...
    def _build_header(self):
        header_frame = customtkinter.CTkFrame(self.left_panel, fg_color=PANEL_BG, corner_radius=10)
        header_frame.grid(row=0, column=0, padx=PAD_OUTER, pady=(PAD_OUTER, 8), sticky="ew")

        customtkinter.CTkLabel(
            header_frame, text=WINDOW_TITLE,
            font=self.font_title, text_color=ACCENT
        ).pack(padx=PAD_INNER, pady=(PAD_INNER, 2))

        customtkinter.CTkLabel(
            header_frame, text=APP_SUBTITLE,
            font=self.font_small, text_color=TEXT_MUTED
        ).pack(padx=PAD_INNER, pady=(0, 2))

        customtkinter.CTkLabel(
            header_frame, text=APP_DESCRIPTION,
            font=self.font_small, text_color=TEXT_MUTED,
    ...
    def _build_buttons(self):
        btn_frame = customtkinter.CTkFrame(self.left_panel, fg_color=APP_BG, corner_radius=0)
        btn_frame.grid(row=2, column=0, padx=PAD_OUTER, pady=(0, PAD_OUTER), sticky="ew")
        ...
        customtkinter.CTkButton(
            ...
            font=self.font_bold,
            fg_color=ACCENT, hover_color="#2BA8D4",
            text_color="#0F172A",
            corner_radius=8, height=42,
        ...
        customtkinter.CTkButton(
            ...
            font=self.font_bold,
            fg_color=PANEL_BG, hover_color="#2E3F50",
            text_color=TEXT_MAIN,
            corner_radius=8, height=42,
            command=self.reset,
    ...
    def _build_problem_tab(self, tab):
        ...
        scroll = customtkinter.CTkScrollableFrame(tab, fg_color=CARD_BG, corner_radius=0)
        scroll.pack(fill="both", expand=True, padx=4, pady=4)

        # — Target Kind —
        customtkinter.CTkLabel(scroll, text="Target Variable Type", font=self.font_section, text_color=ACCENT).pack(anchor="w", padx=PAD_INNER, pady=(PAD_INNER, 2))
        customtkinter.CTkLabel(scroll, text="What kind of value does the model predict?", font=self.font_small, text_color=TEXT_MUTED).pack(anchor="w", padx=PAD_INNER, pady=(0, 6))

        self.radio_continuous = customtkinter.CTkRadioButton(
            scroll, text="Continuous  (Regression — predicts a number)",
            variable=self.target_kind_var, value="continuous",
        ...
        self.radio_binary = customtkinter.CTkRadioButton(
            scroll, text="Binary  (Two-class classification)",
            variable=self.target_kind_var, value="binary",
            font=self.font_normal, text_color=TEXT_MAIN, fg_color=ACCENT
        )
        self.radio_binary.pack(anchor="w", padx=PAD_INNER, pady=PAD_WIDGET)

        self.radio_multiclass = customtkinter.CTkRadioButton(
            scroll, text="Multi-class  (More than two exclusive classes)",
            variable=self.target_kind_var, value="multiclass",
            font=self.font_normal, text_color=TEXT_MAIN, fg_color=ACCENT
        )
        self.radio_multiclass.pack(anchor="w", padx=PAD_INNER, pady=PAD_WIDGET)

        self.radio_multilabel = customtkinter.CTkRadioButton(
            scroll, text="Multi-label  (Sample can belong to multiple classes)",
            variable=self.target_kind_var, value="multilabel",
            font=self.font_normal, text_color=TEXT_MAIN, fg_color=ACCENT
        )
        ...
        # — Label Format —
        customtkinter.CTkLabel(scroll, text="Label / Target Format", font=self.font_section, text_color=ACCENT).pack(anchor="w", padx=PAD_INNER, pady=(4, 2))
        customtkinter.CTkLabel(scroll, text="How are the target labels stored in your dataset?", font=self.font_small, text_color=TEXT_MUTED).pack(anchor="w", padx=PAD_INNER, pady=(0, 6))

        self.radio_numeric_value = customtkinter.CTkRadioButton(
            scroll, text="Numeric value  (e.g. 3.7, 150.0 — for regression)",
            variable=self.label_format_var, value="numeric_value",
            font=self.font_normal, text_color=TEXT_MAIN, fg_color=ACCENT
        )
        self.radio_numeric_value.pack(anchor="w", padx=PAD_INNER, pady=PAD_WIDGET)

        self.radio_integer_ids = customtkinter.CTkRadioButton(
            scroll, text="Integer class IDs  (e.g. 0, 1, 2)",
            variable=self.label_format_var, value="integer_class_ids",
            font=self.font_normal, text_color=TEXT_MAIN, fg_color=ACCENT
        )
        self.radio_integer_ids.pack(anchor="w", padx=PAD_INNER, pady=PAD_WIDGET)

        self.radio_one_hot = customtkinter.CTkRadioButton(
            scroll, text="One-hot encoded  (e.g. [0, 1, 0])",
            variable=self.label_format_var, value="one_hot",
            font=self.font_normal, text_color=TEXT_MAIN, fg_color=ACCENT
        )
        self.radio_one_hot.pack(anchor="w", padx=PAD_INNER, pady=PAD_WIDGET)

        self.radio_multi_binary = customtkinter.CTkRadioButton(
            scroll, text="Multiple binary labels  (e.g. [1, 0, 1, 1])",
            variable=self.label_format_var, value="multiple_binary_labels",
            font=self.font_normal, text_color=TEXT_MAIN, fg_color=ACCENT
        )
        ...
        # — Input Data Type —
        customtkinter.CTkLabel(scroll, text="Main Input Data Type", font=self.font_section, text_color=ACCENT).pack(anchor="w", padx=PAD_INNER, pady=(4, 2))
        customtkinter.CTkLabel(scroll, text="What is the primary format of the model's input features?", font=self.font_small, text_color=TEXT_MUTED).pack(anchor="w", padx=PAD_INNER, pady=(0, 6))

        self.radio_tabular = customtkinter.CTkRadioButton(
            scroll, text="Tabular  (structured rows and columns)",
            variable=self.input_data_type_var, value="tabular",
            font=self.font_normal, text_color=TEXT_MAIN, fg_color=ACCENT
        )
        self.radio_tabular.pack(anchor="w", padx=PAD_INNER, pady=PAD_WIDGET)

        self.radio_image = customtkinter.CTkRadioButton(
            scroll, text="Image  (pixel arrays, photos)",
            variable=self.input_data_type_var, value="image",
            font=self.font_normal, text_color=TEXT_MAIN, fg_color=ACCENT
        )
        self.radio_image.pack(anchor="w", padx=PAD_INNER, pady=PAD_WIDGET)

        self.radio_text_type = customtkinter.CTkRadioButton(
            scroll, text="Text  (natural language, documents)",
            variable=self.input_data_type_var, value="text",
            font=self.font_normal, text_color=TEXT_MAIN, fg_color=ACCENT
        )
        self.radio_text_type.pack(anchor="w", padx=PAD_INNER, pady=PAD_WIDGET)

        self.radio_time_series = customtkinter.CTkRadioButton(
            scroll, text="Time Series  (sequential measurements over time)",
            variable=self.input_data_type_var, value="time_series",
            font=self.font_normal, text_color=TEXT_MAIN, fg_color=ACCENT
        )
        self.radio_time_series.pack(anchor="w", padx=PAD_INNER, pady=PAD_WIDGET)

        self.radio_mixed = customtkinter.CTkRadioButton(
            scroll, text="Mixed  (multiple input modalities combined)",
            variable=self.input_data_type_var, value="mixed",
            font=self.font_normal, text_color=TEXT_MAIN, fg_color=ACCENT
        )
    ...
    def _build_features_tab(self, tab):
        ...
        customtkinter.CTkLabel(scroll, text="Distribution shape:", font=self.font_bold, text_color=TEXT_MAIN).pack(anchor="w", padx=PAD_INNER, pady=(8, 2))

        self.radio_dist_normal = customtkinter.CTkRadioButton(
            scroll, text="Normal  (bell-shaped / Gaussian)",
            variable=self.numerical_distribution_var, value="normal",
            font=self.font_normal, text_color=TEXT_MAIN, fg_color=ACCENT
        )
        ...
        self.radio_dist_bounded = customtkinter.CTkRadioButton(
            scroll, text="Bounded  (fixed min and max range)",
            variable=self.numerical_distribution_var, value="bounded",
            font=self.font_normal, text_color=TEXT_MAIN, fg_color=ACCENT
        )
        ...
        self.radio_dist_skewed = customtkinter.CTkRadioButton(
            scroll, text="Skewed  (long tail on one side)",
            variable=self.numerical_distribution_var, value="skewed",
            font=self.font_normal, text_color=TEXT_MAIN, fg_color=ACCENT
        )
        ...
        self.radio_dist_unknown = customtkinter.CTkRadioButton(
            scroll, text="Unknown  (not sure about the distribution)",
            variable=self.numerical_distribution_var, value="unknown",
            font=self.font_normal, text_color=TEXT_MAIN, fg_color=ACCENT
        )
        ...
        customtkinter.CTkLabel(scroll, text="Nonlinear relationships:", font=self.font_bold, text_color=TEXT_MAIN).pack(anchor="w", padx=PAD_INNER, pady=(8, 2))

        self.radio_nonlinear_yes = customtkinter.CTkRadioButton(
            scroll, text="Yes  (features interact nonlinearly with the target)",
            variable=self.nonlinear_relationships_var, value="yes",
            font=self.font_normal, text_color=TEXT_MAIN, fg_color=ACCENT
        )
        ...
        self.radio_nonlinear_no = customtkinter.CTkRadioButton(
            scroll, text="No  (relationships are roughly linear)",
            variable=self.nonlinear_relationships_var, value="no",
            font=self.font_normal, text_color=TEXT_MAIN, fg_color=ACCENT
        )
        self.radio_nonlinear_no.pack(anchor="w", padx=PAD_INNER + 12, pady=PAD_WIDGET)

        customtkinter.CTkFrame(scroll, height=2, fg_color=PANEL_BG).pack(fill="x", padx=PAD_INNER, pady=PAD_INNER)

        # ---- Categorical Features ----
        customtkinter.CTkLabel(scroll, text="Categorical Features", font=self.font_section, text_color=ACCENT).pack(anchor="w", padx=PAD_INNER, pady=(4, 4))

        self.check_categorical = customtkinter.CTkCheckBox(
            scroll, text="Dataset contains categorical features",
            variable=self.has_categorical_var,
            font=self.font_normal, text_color=TEXT_MAIN, fg_color=ACCENT, checkmark_color="#0F172A"
        )
        self.check_categorical.pack(anchor="w", padx=PAD_INNER, pady=PAD_WIDGET)

        customtkinter.CTkLabel(scroll, text="Cardinality:", font=self.font_bold, text_color=TEXT_MAIN).pack(anchor="w", padx=PAD_INNER, pady=(8, 2))

        self.radio_card_low = customtkinter.CTkRadioButton(
            scroll, text="Low  (few unique categories per column)",
            variable=self.categorical_cardinality_var, value="low",
            font=self.font_normal, text_color=TEXT_MAIN, fg_color=ACCENT
        )
        self.radio_card_low.pack(anchor="w", padx=PAD_INNER + 12, pady=PAD_WIDGET)

        self.radio_card_high = customtkinter.CTkRadioButton(
            scroll, text="High  (many unique categories per column)",
            variable=self.categorical_cardinality_var, value="high",
            font=self.font_normal, text_color=TEXT_MAIN, fg_color=ACCENT
        )
        self.radio_card_high.pack(anchor="w", padx=PAD_INNER + 12, pady=PAD_WIDGET)

        customtkinter.CTkLabel(scroll, text="Order type:", font=self.font_bold, text_color=TEXT_MAIN).pack(anchor="w", padx=PAD_INNER, pady=(8, 2))

        self.radio_nominal = customtkinter.CTkRadioButton(
            scroll, text="Nominal  (no natural order, e.g. city names, colors)",
            variable=self.categorical_order_var, value="nominal",
            font=self.font_normal, text_color=TEXT_MAIN, fg_color=ACCENT
        )
        self.radio_nominal.pack(anchor="w", padx=PAD_INNER + 12, pady=PAD_WIDGET)

        self.radio_ordinal = customtkinter.CTkRadioButton(
            scroll, text="Ordinal  (meaningful order, e.g. low / medium / high)",
            variable=self.categorical_order_var, value="ordinal",
            font=self.font_normal, text_color=TEXT_MAIN, fg_color=ACCENT
        )
        self.radio_ordinal.pack(anchor="w", padx=PAD_INNER + 12, pady=PAD_WIDGET)

        customtkinter.CTkLabel(scroll, text="Rare categories:", font=self.font_bold, text_color=TEXT_MAIN).pack(anchor="w", padx=PAD_INNER, pady=(8, 2))

        self.radio_rare_yes = customtkinter.CTkRadioButton(
            scroll, text="Yes  (some categories appear very rarely)",
            variable=self.rare_categories_var, value="yes",
            font=self.font_normal, text_color=TEXT_MAIN, fg_color=ACCENT
        )
        self.radio_rare_yes.pack(anchor="w", padx=PAD_INNER + 12, pady=PAD_WIDGET)

        self.radio_rare_no = customtkinter.CTkRadioButton(
            scroll, text="No  (all categories appear reasonably often)",
            variable=self.rare_categories_var, value="no",
            font=self.font_normal, text_color=TEXT_MAIN, fg_color=ACCENT
        )
        self.radio_rare_no.pack(anchor="w", padx=PAD_INNER + 12, pady=PAD_WIDGET)

        customtkinter.CTkLabel(scroll, text="Noisy category names:", font=self.font_bold, text_color=TEXT_MAIN).pack(anchor="w", padx=PAD_INNER, pady=(8, 2))

        self.radio_noisy_cat_yes = customtkinter.CTkRadioButton(
            scroll, text="Yes  (typos, inconsistent capitalization, duplicates)",
            variable=self.noisy_categories_var, value="yes",
            font=self.font_normal, text_color=TEXT_MAIN, fg_color=ACCENT
        )
        ...
        self.radio_noisy_cat_no = customtkinter.CTkRadioButton(
            scroll, text="No  (category names are clean and consistent)",
            variable=self.noisy_categories_var, value="no",
            font=self.font_normal, text_color=TEXT_MAIN, fg_color=ACCENT
        )
        ...
        # ---- Date-Time Features ----
        customtkinter.CTkLabel(scroll, text="Date-Time Features", font=self.font_section, text_color=ACCENT).pack(anchor="w", padx=PAD_INNER, pady=(4, 4))

        self.check_datetime = customtkinter.CTkCheckBox(
            scroll, text="Dataset contains date / time columns",
            variable=self.has_datetime_var,
            font=self.font_normal, text_color=TEXT_MAIN, fg_color=ACCENT, checkmark_color="#0F172A"
        )
        self.check_datetime.pack(anchor="w", padx=PAD_INNER, pady=PAD_WIDGET)

        customtkinter.CTkLabel(scroll, text="Seasonality:", font=self.font_bold, text_color=TEXT_MAIN).pack(anchor="w", padx=PAD_INNER, pady=(8, 2))

        self.radio_season_yes = customtkinter.CTkRadioButton(
            scroll, text="Yes  (seasonal patterns exist)",
            variable=self.seasonality_var, value="yes",
            font=self.font_normal, text_color=TEXT_MAIN, fg_color=ACCENT
        )
        self.radio_season_yes.pack(anchor="w", padx=PAD_INNER + 12, pady=PAD_WIDGET)

        self.radio_season_no = customtkinter.CTkRadioButton(
            scroll, text="No  (no significant seasonal patterns)",
            variable=self.seasonality_var, value="no",
            font=self.font_normal, text_color=TEXT_MAIN, fg_color=ACCENT
        )
        self.radio_season_no.pack(anchor="w", padx=PAD_INNER + 12, pady=PAD_WIDGET)

        customtkinter.CTkLabel(scroll, text="Time order matters:", font=self.font_bold, text_color=TEXT_MAIN).pack(anchor="w", padx=PAD_INNER, pady=(8, 2))

        self.radio_timeord_yes = customtkinter.CTkRadioButton(
            scroll, text="Yes  (past → future ordering is meaningful)",
            variable=self.time_order_var, value="yes",
            font=self.font_normal, text_color=TEXT_MAIN, fg_color=ACCENT
        )
        self.radio_timeord_yes.pack(anchor="w", padx=PAD_INNER + 12, pady=PAD_WIDGET)

        self.radio_timeord_no = customtkinter.CTkRadioButton(
            scroll, text="No  (temporal ordering is not critical)",
            variable=self.time_order_var, value="no",
            font=self.font_normal, text_color=TEXT_MAIN, fg_color=ACCENT
        )
        self.radio_timeord_no.pack(anchor="w", padx=PAD_INNER + 12, pady=PAD_WIDGET)

        customtkinter.CTkLabel(scroll, text="Cyclic time patterns:", font=self.font_bold, text_color=TEXT_MAIN).pack(anchor="w", padx=PAD_INNER, pady=(8, 2))

        self.radio_cyclic_yes = customtkinter.CTkRadioButton(
            scroll, text="Yes  (e.g. hour of day, day of week, month)",
            variable=self.cyclic_time_var, value="yes",
            font=self.font_normal, text_color=TEXT_MAIN, fg_color=ACCENT
        )
        self.radio_cyclic_yes.pack(anchor="w", padx=PAD_INNER + 12, pady=PAD_WIDGET)

        self.radio_cyclic_no = customtkinter.CTkRadioButton(
            scroll, text="No  (no cyclic time features)",
            variable=self.cyclic_time_var, value="no",
            font=self.font_normal, text_color=TEXT_MAIN, fg_color=ACCENT
        )
        ...
        # ---- Text Features ----
        customtkinter.CTkLabel(scroll, text="Text Features", font=self.font_section, text_color=ACCENT).pack(anchor="w", padx=PAD_INNER, pady=(4, 4))

        self.check_text = customtkinter.CTkCheckBox(
            scroll, text="Dataset contains text / natural language columns",
            variable=self.has_text_var,
            font=self.font_normal, text_color=TEXT_MAIN, fg_color=ACCENT, checkmark_color="#0F172A"
        )
        self.check_text.pack(anchor="w", padx=PAD_INNER, pady=PAD_WIDGET)

        customtkinter.CTkLabel(scroll, text="Text length:", font=self.font_bold, text_color=TEXT_MAIN).pack(anchor="w", padx=PAD_INNER, pady=(8, 2))

        self.radio_text_short = customtkinter.CTkRadioButton(
            scroll, text="Short  (tweets, titles, short sentences)",
            variable=self.text_length_var, value="short",
            font=self.font_normal, text_color=TEXT_MAIN, fg_color=ACCENT
        )
        self.radio_text_short.pack(anchor="w", padx=PAD_INNER + 12, pady=PAD_WIDGET)

        self.radio_text_long = customtkinter.CTkRadioButton(
            scroll, text="Long  (paragraphs, articles, full documents)",
            variable=self.text_length_var, value="long",
            font=self.font_normal, text_color=TEXT_MAIN, fg_color=ACCENT
        )
        self.radio_text_long.pack(anchor="w", padx=PAD_INNER + 12, pady=PAD_WIDGET)

        customtkinter.CTkLabel(scroll, text="Text dataset size:", font=self.font_bold, text_color=TEXT_MAIN).pack(anchor="w", padx=PAD_INNER, pady=(8, 2))

        self.radio_tsize_small = customtkinter.CTkRadioButton(
            scroll, text="Small  (limited labelled text data)",
            variable=self.text_dataset_size_var, value="small",
            font=self.font_normal, text_color=TEXT_MAIN, fg_color=ACCENT
        )
        ...
        self.radio_tsize_large = customtkinter.CTkRadioButton(
            scroll, text="Large  (abundant labelled text data)",
            variable=self.text_dataset_size_var, value="large",
            font=self.font_normal, text_color=TEXT_MAIN, fg_color=ACCENT
        )
        self.radio_tsize_large.pack(anchor="w", padx=PAD_INNER + 12, pady=PAD_WIDGET)

        customtkinter.CTkLabel(scroll, text="Word order importance:", font=self.font_bold, text_color=TEXT_MAIN).pack(anchor="w", padx=PAD_INNER, pady=(8, 2))

        self.radio_wordord_yes = customtkinter.CTkRadioButton(
            scroll, text="Yes  (sentence structure and word order matter)",
            variable=self.word_order_var, value="yes",
            font=self.font_normal, text_color=TEXT_MAIN, fg_color=ACCENT
        )
        self.radio_wordord_yes.pack(anchor="w", padx=PAD_INNER + 12, pady=PAD_WIDGET)

        self.radio_wordord_no = customtkinter.CTkRadioButton(
            scroll, text="No  (bag-of-words approach is acceptable)",
            variable=self.word_order_var, value="no",
            font=self.font_normal, text_color=TEXT_MAIN, fg_color=ACCENT
        )
        ...
        # ---- Image Features ----
        customtkinter.CTkLabel(scroll, text="Image Features", font=self.font_section, text_color=ACCENT).pack(anchor="w", padx=PAD_INNER, pady=(4, 4))

        self.check_image = customtkinter.CTkCheckBox(
            scroll, text="Dataset contains image data",
            variable=self.has_image_var,
            font=self.font_normal, text_color=TEXT_MAIN, fg_color=ACCENT, checkmark_color="#0F172A"
        )
        self.check_image.pack(anchor="w", padx=PAD_INNER, pady=PAD_WIDGET)

        customtkinter.CTkLabel(scroll, text="Image dataset size:", font=self.font_bold, text_color=TEXT_MAIN).pack(anchor="w", padx=PAD_INNER, pady=(8, 2))

        self.radio_isize_small = customtkinter.CTkRadioButton(
            scroll, text="Small  (fewer than ~5,000 images)",
            variable=self.image_dataset_size_var, value="small",
            font=self.font_normal, text_color=TEXT_MAIN, fg_color=ACCENT
        )
        ...
        self.radio_isize_large = customtkinter.CTkRadioButton(
            scroll, text="Large  (tens of thousands of images or more)",
            variable=self.image_dataset_size_var, value="large",
            font=self.font_normal, text_color=TEXT_MAIN, fg_color=ACCENT
        )
        self.radio_isize_large.pack(anchor="w", padx=PAD_INNER + 12, pady=PAD_WIDGET)

        customtkinter.CTkLabel(scroll, text="Image augmentation needed:", font=self.font_bold, text_color=TEXT_MAIN).pack(anchor="w", padx=PAD_INNER, pady=(8, 2))

        self.radio_aug_yes = customtkinter.CTkRadioButton(
            scroll, text="Yes  (augmentation should be applied during training)",
            variable=self.image_augmentation_var, value="yes",
            font=self.font_normal, text_color=TEXT_MAIN, fg_color=ACCENT
        )
        self.radio_aug_yes.pack(anchor="w", padx=PAD_INNER + 12, pady=PAD_WIDGET)

        self.radio_aug_no = customtkinter.CTkRadioButton(
            scroll, text="No  (use images as-is without augmentation)",
            variable=self.image_augmentation_var, value="no",
            font=self.font_normal, text_color=TEXT_MAIN, fg_color=ACCENT
        )
        self.radio_aug_no.pack(anchor="w", padx=PAD_INNER + 12, pady=(PAD_WIDGET, PAD_INNER))
    ...
    def _build_quality_tab(self, tab):
        ...
        # — Missing Values —
        customtkinter.CTkLabel(scroll, text="Missing Values", font=self.font_section, text_color=ACCENT).pack(anchor="w", padx=PAD_INNER, pady=(PAD_INNER, 2))
        customtkinter.CTkLabel(scroll, text="Does your dataset contain NaN or null values?", font=self.font_small, text_color=TEXT_MUTED).pack(anchor="w", padx=PAD_INNER, pady=(0, 6))

        self.radio_mv_yes = customtkinter.CTkRadioButton(
            scroll, text="Yes  (some cells are missing)",
            variable=self.missing_values_var, value="yes",
            font=self.font_normal, text_color=TEXT_MAIN, fg_color=ACCENT
        )
        self.radio_mv_yes.pack(anchor="w", padx=PAD_INNER, pady=PAD_WIDGET)

        self.radio_mv_no = customtkinter.CTkRadioButton(
            scroll, text="No  (the dataset is complete)",
            variable=self.missing_values_var, value="no",
            font=self.font_normal, text_color=TEXT_MAIN, fg_color=ACCENT
        )
        ...
        # — Class Imbalance —
        customtkinter.CTkLabel(scroll, text="Class Imbalance", font=self.font_section, text_color=ACCENT).pack(anchor="w", padx=PAD_INNER, pady=(4, 2))
        customtkinter.CTkLabel(scroll, text="Is one class much more frequent than the others?", font=self.font_small, text_color=TEXT_MUTED).pack(anchor="w", padx=PAD_INNER, pady=(0, 6))

        self.radio_ci_yes = customtkinter.CTkRadioButton(
            scroll, text="Yes  (significant class imbalance exists)",
            variable=self.class_imbalance_var, value="yes",
            font=self.font_normal, text_color=TEXT_MAIN, fg_color=ACCENT
        )
        self.radio_ci_yes.pack(anchor="w", padx=PAD_INNER, pady=PAD_WIDGET)

        self.radio_ci_no = customtkinter.CTkRadioButton(
            scroll, text="No  (classes are roughly balanced)",
            variable=self.class_imbalance_var, value="no",
            font=self.font_normal, text_color=TEXT_MAIN, fg_color=ACCENT
        )
        ...
        # — Noisy Labels —
        customtkinter.CTkLabel(scroll, text="Noisy Labels", font=self.font_section, text_color=ACCENT).pack(anchor="w", padx=PAD_INNER, pady=(4, 2))
        customtkinter.CTkLabel(scroll, text="Do you suspect some training labels are incorrect?", font=self.font_small, text_color=TEXT_MUTED).pack(anchor="w", padx=PAD_INNER, pady=(0, 6))

        self.radio_nl_yes = customtkinter.CTkRadioButton(
            scroll, text="Yes  (some labels may be wrong or noisy)",
            variable=self.noisy_labels_var, value="yes",
            font=self.font_normal, text_color=TEXT_MAIN, fg_color=ACCENT
        )
        self.radio_nl_yes.pack(anchor="w", padx=PAD_INNER, pady=PAD_WIDGET)

        self.radio_nl_no = customtkinter.CTkRadioButton(
            scroll, text="No  (labels are reliable and accurate)",
            variable=self.noisy_labels_var, value="no",
            font=self.font_normal, text_color=TEXT_MAIN, fg_color=ACCENT
        )
        ...
        # — Data Leakage —
        customtkinter.CTkLabel(scroll, text="Data Leakage Risk", font=self.font_section, text_color=ACCENT).pack(anchor="w", padx=PAD_INNER, pady=(4, 2))
        customtkinter.CTkLabel(
            scroll,
            text="Could any feature contain information not available at prediction time?",
            font=self.font_small, text_color=TEXT_MUTED, wraplength=580
        ).pack(anchor="w", padx=PAD_INNER, pady=(0, 6))

        self.radio_dl_yes = customtkinter.CTkRadioButton(
            scroll, text="Yes  (possible data leakage identified)",
            variable=self.data_leakage_var, value="yes",
            font=self.font_normal, text_color=TEXT_MAIN, fg_color=ACCENT
        )
        self.radio_dl_yes.pack(anchor="w", padx=PAD_INNER, pady=PAD_WIDGET)

        self.radio_dl_no = customtkinter.CTkRadioButton(
            scroll, text="No  (features are safe and leak-free)",
            variable=self.data_leakage_var, value="no",
            font=self.font_normal, text_color=TEXT_MAIN, fg_color=ACCENT
        )
    ...
    def _build_training_tab(self, tab):
        ...
        # — Overfitting —
        customtkinter.CTkLabel(scroll, text="Overfitting", font=self.font_section, text_color=ACCENT).pack(anchor="w", padx=PAD_INNER, pady=(4, 2))
        customtkinter.CTkLabel(scroll, text="Training accuracy is high but validation accuracy is much lower.", font=self.font_small, text_color=TEXT_MUTED).pack(anchor="w", padx=PAD_INNER, pady=(0, 6))

        self.radio_of_yes = customtkinter.CTkRadioButton(
            scroll, text="Yes  (model is overfitting)",
            variable=self.overfitting_var, value="yes",
            font=self.font_normal, text_color=TEXT_MAIN, fg_color=ACCENT
        )
        self.radio_of_yes.pack(anchor="w", padx=PAD_INNER, pady=PAD_WIDGET)

        self.radio_of_no = customtkinter.CTkRadioButton(
            scroll, text="No  (not observed)",
            variable=self.overfitting_var, value="no",
            font=self.font_normal, text_color=TEXT_MAIN, fg_color=ACCENT
        )
        ...
        # — Underfitting —
        customtkinter.CTkLabel(scroll, text="Underfitting", font=self.font_section, text_color=ACCENT).pack(anchor="w", padx=PAD_INNER, pady=(4, 2))
        customtkinter.CTkLabel(scroll, text="Both training and validation performance are poor.", font=self.font_small, text_color=TEXT_MUTED).pack(anchor="w", padx=PAD_INNER, pady=(0, 6))

        self.radio_uf_yes = customtkinter.CTkRadioButton(
            scroll, text="Yes  (model is underfitting)",
            variable=self.underfitting_var, value="yes",
            font=self.font_normal, text_color=TEXT_MAIN, fg_color=ACCENT
        )
        ...
        self.radio_uf_no = customtkinter.CTkRadioButton(
            scroll, text="No  (not observed)",
            variable=self.underfitting_var, value="no",
            font=self.font_normal, text_color=TEXT_MAIN, fg_color=ACCENT
        )
        ...
        # — Unstable Training —
        customtkinter.CTkLabel(scroll, text="Unstable Training", font=self.font_section, text_color=ACCENT).pack(anchor="w", padx=PAD_INNER, pady=(4, 2))
        customtkinter.CTkLabel(scroll, text="Loss oscillates wildly, spikes, or diverges during training.", font=self.font_small, text_color=TEXT_MUTED).pack(anchor="w", padx=PAD_INNER, pady=(0, 6))

        self.radio_ut_yes = customtkinter.CTkRadioButton(
            scroll, text="Yes  (training is unstable)",
            variable=self.unstable_training_var, value="yes",
            font=self.font_normal, text_color=TEXT_MAIN, fg_color=ACCENT
        )
        self.radio_ut_yes.pack(anchor="w", padx=PAD_INNER, pady=PAD_WIDGET)

        self.radio_ut_no = customtkinter.CTkRadioButton(
            scroll, text="No  (training is stable)",
            variable=self.unstable_training_var, value="no",
            font=self.font_normal, text_color=TEXT_MAIN, fg_color=ACCENT
        )
        ...
        # — Slow Training —
        customtkinter.CTkLabel(scroll, text="Slow Training", font=self.font_section, text_color=ACCENT).pack(anchor="w", padx=PAD_INNER, pady=(4, 2))
        customtkinter.CTkLabel(scroll, text="Training takes much longer than expected per epoch.", font=self.font_small, text_color=TEXT_MUTED).pack(anchor="w", padx=PAD_INNER, pady=(0, 6))

        self.radio_st_yes = customtkinter.CTkRadioButton(
            scroll, text="Yes  (training is noticeably slow)",
            variable=self.slow_training_var, value="yes",
            font=self.font_normal, text_color=TEXT_MAIN, fg_color=ACCENT
        )
        self.radio_st_yes.pack(anchor="w", padx=PAD_INNER, pady=PAD_WIDGET)

        self.radio_st_no = customtkinter.CTkRadioButton(
            scroll, text="No  (training speed is acceptable)",
            variable=self.slow_training_var, value="no",
            font=self.font_normal, text_color=TEXT_MAIN, fg_color=ACCENT
        )
    ...
    def analyze(self):
        ...
        self.engine.declare(DatasetFact(feature_sample_ratio=self.feature_sample_ratio_var.get()))
        ...
        self.engine.declare(FeatureFact(numerical_distribution=self.numerical_distribution_var.get()))
        self.engine.declare(FeatureFact(numerical_outliers=self.numerical_outliers_var.get()))
        self.engine.declare(FeatureFact(nonlinear_relationships=self.nonlinear_relationships_var.get()))

        self.engine.declare(FeatureFact(has_categorical=self.has_categorical_var.get()))
        self.engine.declare(FeatureFact(categorical_cardinality=self.categorical_cardinality_var.get()))
        self.engine.declare(FeatureFact(categorical_order=self.categorical_order_var.get()))
        self.engine.declare(FeatureFact(rare_categories=self.rare_categories_var.get()))
        self.engine.declare(FeatureFact(noisy_categories=self.noisy_categories_var.get()))
        ...
        self.engine.declare(FeatureFact(text_dataset_size=self.text_dataset_size_var.get()))
        ...
        self.engine.declare(FeatureFact(image_dataset_size=self.image_dataset_size_var.get()))
        self.engine.declare(FeatureFact(image_augmentation=self.image_augmentation_var.get()))
        ...
        self.engine.declare(TrainingFact(unstable_training=self.unstable_training_var.get()))
